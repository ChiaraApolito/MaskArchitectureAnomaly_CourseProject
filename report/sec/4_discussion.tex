...
The pixel-based ERFNet baseline is weak across the board: its best score,
Max-Logit, tops out at $38.3$ AuPRC on RA-21 and collapses to single digits on
the obstacle and Lost\,\&\,Found tracks (\cref{tab:anomaly}), because a
per-pixel softmax is systematically over-confident on unknown inputs, so
...
(5) \emph{Reporting gaps}: the Task 4 per-class comparison, the remaining
per-stage mIoU values (Stage 0, Stage 2 and the head-only baseline) and the MSP
temperature sweep are still pending and flagged in \cref{sec:results} for
completion. Finally,
