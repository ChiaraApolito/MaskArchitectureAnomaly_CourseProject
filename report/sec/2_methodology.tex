...
\subsection{Task 7: ERFNet pixel-based baseline}
\label{sec:method-task7}
ERFNet~\cite{erfnet} is a lightweight, fully convolutional encoder--decoder
network designed for real-time semantic segmentation. It serves as a pixel-based
baseline against which to measure the mask-classification approach. Unlike EoMT,
ERFNet outputs per-pixel class distributions directly without any query
mechanism or set-based decoding. The ERFNet model operates at its native
resolution without intermediate upscaling, making it a pure pixel-level
classifier that cannot leverage structural priors or object-level reasoning.
...
segmentation network's outputs into a per-pixel anomaly score. For both pixel-based
(ERFNet) and mask-based (EoMT) networks, we extract semantic logits $z_c(h,w)$ and apply
post-hoc scoring. From these logits with temperature $T$ we use:
\begin{align}
